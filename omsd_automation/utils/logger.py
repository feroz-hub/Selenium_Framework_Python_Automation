...
#         # Attach Once per run file handler to root logger
...
# ):
#     """
#     Configure the root logger once per test run.
#     Prevents duplicate handlers/messages across pytest runs.
#     """
#     global _RUN_LOG_FILE, _ROOT_CONFIGURED
#
#     if _ROOT_CONFIGURED:
...
#     root.setLevel(logging.DEBUG)
#
#     # 🚨 Check handlers to prevent duplicates
#     if not root.handlers:
#         # File handler
#         file_handler = logging.FileHandler(_RUN_LOG_FILE, mode="a", encoding="utf-8")
#         file_handler.setLevel(file_level)
...
#         root.addHandler(console_handler)
#
#     # 🛑 Print log file info only once per process
#     if not getattr(root, "_run_log_announced", False):
#         root.info(f"📁 Unified log file for this run: {_RUN_LOG_FILE}")
#         root._run_log_announced = True
#
...
#
#
# def setup_test_logging(test_name: str = "default", log_dir: str = str(_DEFAULT_LOG_DIR)) -> TestLogger:
...
# Thread-safety for one-time configuration
...
def _ensure_root_logger(
    ...
    """
    Configure the root logger once per test run.
    Prevents duplicate handlers/messages across pytest runs and threads.
    """
    global _RUN_LOG_FILE, _ROOT_CONFIGURED
    ...
    with _CONFIG_LOCK:
        ...
        # Compute run log file only once
        ...
        # Announce the unified log file only once per process
        if not getattr(root, "_run_log_announced", False):
            root.info(f"📁 Unified log file for this run: {_RUN_LOG_FILE}")
            root._run_log_announced = True
...
# Additional function to reset logger state (useful for testing)
def reset_logger_state():
    """Reset the global logger state. Use with caution, mainly for testing."""
    global _RUN_LOG_FILE, _ROOT_CONFIGURED
    _RUN_LOG_FILE = None
