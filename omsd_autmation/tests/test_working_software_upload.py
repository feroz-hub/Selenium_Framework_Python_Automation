...
class TestWorkingSoftwareUpload:
    """
    Enhanced test suite for software upload functionality.
    
    This class provides optimized, maintainable tests for software upload operations
    including popup management, file uploads, and multi-product testing with
    ...
    def setup_test_logging(self) -> None:
        ...
        self.test_logger: TestLogger = setup_test_logging("working_software_upload")
    ...
    @pytest.mark.smoke
    def test_upload_popup_cancel_functionality(self, software_uploader_login: WebDriver, software_page: SoftwarePage) -> None:
        """
        Test upload popup cancel functionality.
        
        This smoke test verifies that the cancel button works correctly
        and properly closes the upload popup.
        
        Args:
            software_uploader_login: Authenticated WebDriver instance
            software_page: SoftwarePage instance
        """
        self.test_logger.test_start("test_upload_popup_cancel_functionality")
        ...
        self.test_logger.step("Testing cancel button functionality")
        software_page.cancel_upload_popup()

        # Verify popup is closed
        assert not software_page.is_upload_popup_visible(), "Upload popup should be closed after cancel"
        self.test_logger.verification("Upload popup closed after cancel", True)

        # Verify we're back to software list
        assert software_page.is_software_list_opened(), "Should return to software list after cancel"
        self.test_logger.verification("Returned to software list", True)

        self.test_logger.test_end("test_upload_popup_cancel_functionality", True)

    @pytest.mark.integration
    def test_upload_popup_without_file_selection(self, software_uploader_login: WebDriver, software_page: SoftwarePage) -> None:
        """
        Test upload popup behavior when no file is selected.
        
        This integration test verifies the system's behavior when attempting
        to submit the upload form without selecting a file.
        
        Args:
            software_uploader_login: Authenticated WebDriver instance
            software_page: SoftwarePage instance
        """
        self.test_logger.test_start("test_upload_popup_without_file_selection")
        ...
        # Verify submit button is present but test its behavior
        assert software_page.is_submit_button_visible(), "Submit button not available"
        self.test_logger.verification("Submit button available", True)

        # Note: Actual submission without file would depend on application behavior
        # For now, we just verify the button is present and clickable
        self.test_logger.step("Verifying submit button is clickable")
        
        # Clean up without submitting
        self._cleanup_upload_popup(software_page)

        self.test_logger.test_end("test_upload_popup_without_file_selection", True)
